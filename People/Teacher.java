...
public class Teacher extends Employee implements Serializable{
    ...
    public void addCourse(Course course) {
        if (!courses.contains(course)) { // Проверяем, есть ли уже курс
            courses.add(course);
        } else {
    ...
    public boolean approveStudent(Course course, Student student) {
    ...
    public String showRating() {
        if (numOfRatedStudent == 0) {
            return "Преподаватель еще не был оценен.";
        }
        return "Средний рейтинг преподавателя " + this.getName() + " составляет " + this.getRate() + " из 10.";
    }
    ...
    public void showInfo(){
        ...
        System.out.println("Average rate " + this.getRate());
